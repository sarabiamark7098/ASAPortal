<template>
  <div class="p-4 sm:p-6">
    <FullScreenLoader :visible="form.loading" message="Loading Janitorial Service Request..." />
          ...
          <div class="relative flex-1 overflow-y-auto">
            ...
            <div class="overflow-x-auto max-h-[60vh]">
              <table class="w-full min-w-max table-auto border border-gray-200 text-sm">
                ...
                <tbody>
                  <tr
                    v-for="item in JanitorialRequests"
                    :key="item.id"
...
import ApproveJanitorialServices from '@/views/Admin/Requests/JanitorialServicesRequest/ApproveJanitorialServices.vue'
import PrintTransaction from '@/views/Admin/Requests/JanitorialServicesRequest/PrintTransaction.vue'
...
const JanitorialRequests = computed(() => form.janitorialRequests)
...
const columns = [
  ...
  { field: 'requested_date', header: 'Requested Date' },
  { field: 'requested_time', header: 'Requested Time' },
...
const requestSummary = computed(() => {
  ...
  return {
    ...
    'Requested Date': form.formatDate(req.requested_date) ?? 'N/A',
    'Requested Time': form.formatTime(req.requested_time) ?? 'N/A',
    'Place of Assignment': req.location ?? 'N/A',
    'Number of Utility Workers': req.count_utility ?? 'N/A',
...
function loadRequests(page = currentPage.value) {
  ...
  form
    .getJanitorialRequests(authStore.token, page, form.rows, search, sortBy, sortDir)
    .catch((err) => {
      console.error('Failed to load Janitorial requests:', err)
    })
    .finally(() => {
      form.loading2 = false
    })
