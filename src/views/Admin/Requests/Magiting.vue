<template>
  <div class="p-4 sm:p-6">
    <FullScreenLoader :visible="form.loading" message="Loading Magiting Conference Request..." />

    <div v-if="!form.loading">
      <!-- Header -->
      <div class="mb-4">
        <h1 class="text-2xl sm:text-3xl font-bold">Magiting Conference Requests</h1>
        <span class="text-sm sm:text-base">Manage Magiting Conference Requests</span>
      </div>
          ...
          <div class="relative flex-1 overflow-y-auto">
            ...
            <div class="overflow-x-auto max-h-[60vh]">
              <table class="w-full min-w-max table-auto border border-gray-200 text-sm">
                ...
                <tbody>
                  <tr
                    v-for="item in conferenceRequests"
                    :key="item.id"
...
import ApproveConference from '@/views/Admin/Requests/ConferenceRequest/ApproveConference.vue'
import NonAvailability from '@/views/Admin/Requests/ConferenceRequest/NonAvailability.vue'
import PrintTransaction from '@/views/Admin/Requests/ConferenceRequest/PrintTransaction.vue'
...
const conferenceRequests = computed(() => form.conferenceRequests)
...
const requestSummary = computed(() => {
  ...
  return {
    ...
    'Start Time': req.requested_time_start ?? 'N/A',
    'End Time': req.requested_time_end ?? 'N/A',
    Purpose: req.purpose ?? 'N/A',
    'Focal Person': req.focal ?? 'N/A',
    'Number of Persons': req.number_of_persons ?? 'N/A',
...
function loadRequests(page = currentPage.value) {
  ...
  form
    .getRoomRequests(authStore.token, page, form.rows, search, sortBy, sortDir, room)
    .catch((err) => {
      console.error('Failed to load Magiting Conference requests:', err)
    })
    .finally(() => {
      form.loading2 = false
    })
