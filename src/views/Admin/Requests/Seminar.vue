<template>
  <div class="p-4 sm:p-6">
    <FullScreenLoader :visible="form.loading" message="Loading Seminar Hall Request..." />

    <div v-if="!form.loading">
      <!-- Header -->
      <div class="mb-4">
        <h1 class="text-2xl sm:text-3xl font-bold">Seminar Hall Requests</h1>
        <span class="text-sm sm:text-base">Manage Seminar Hall Requests</span>
      </div>
...
function loadRequests(page = currentPage.value) {
  ...
  form
    ...
    .catch((err) => {
      console.error('Failed to load seminar hall requests:', err)
    })
    .finally(() => {
      form.loading2 = false
    })
