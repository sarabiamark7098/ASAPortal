<template>
  <div class="p-4 sm:p-6">
    <FullScreenLoader :visible="form.loading" message="Loading Air Transport Order Request..." />
          ...
          <div class="relative flex-1 overflow-y-auto">
            ...
            <div class="overflow-x-auto max-h-[60vh]">
              <table class="w-full min-w-max table-auto border border-gray-200 text-sm">
                ...
                <tbody>
                  <tr
                    v-for="item in AirTransportRequests"
                    :key="item.id"
...
import PrintTransaction from '@/views/Admin/Requests/AirTransportOrderRequest/PrintTransaction.vue'
...
const AirTransportRequests = computed(() => form.airTransportRequests)
...
function loadRequests(page = currentPage.value) {
  ...
  form
    .getAirTransportRequests(authStore.token, page, form.rows, search, sortBy, sortDir)
    .catch((err) => {
      console.error('Failed to load Air Transport requests:', err)
    })
    .finally(() => {
      form.loading2 = false
    })
