<template>
  <div class="p-4 sm:p-6">
    <FullScreenLoader :visible="form.loading" message="Loading Overnight Parking Request..." />

    <div v-if="!form.loading">
      <!-- Header -->
      <div class="mb-4">
        <h1 class="text-2xl sm:text-3xl font-bold">Overnight Parking Requests</h1>
        <span class="text-sm sm:text-base">Manage Overnight Parking Requests</span>
      </div>

      <!-- Conditional Edit/Print Modes -->
      <div v-if="form.editMode" class="flex flex-col md:flex-row gap-4">
        <ApproveOvernightParking
          ...
          <div class="relative flex-1 overflow-y-auto">
            ...
            <div class="overflow-x-auto max-h-[60vh]">
              <table class="w-full min-w-max table-auto border border-gray-200 text-sm">
                ...
                <tbody>
                  <tr
                    v-for="item in overnightParkingRequests"
                    :key="item.id"
                  ...
                  >
                    ...
                    <td class="px-2 sm:px-4 py-2">{{ item.requester_name }}</td>
...
import ApproveOvernightParking from '@/views/Admin/Requests/OvernightParkingRequest/ApproveOvernightParking.vue'
import PrintTransaction from '@/views/Admin/Requests/OvernightParkingRequest/PrintTransaction.vue'
...
const overnightParkingRequests = computed(() => form.overnightParkingRequests)
...
const columns = [
  ...
  { field: 'requester_name', header: 'Requester' },
...
const requestSummary = computed(() => {
  ...
  return {
    ...
    'Plate Number': req.plate_number ?? 'N/A',
    'Vehicle Model': req.model ?? 'N/A',
...
function loadRequests(page = currentPage.value) {
  ...
  form
    .getOvernightParkingRequests(authStore.token, page, form.rows, search, sortBy, sortDir)
    .catch((err) => {
      console.error('Failed to load overnight parking requests:', err)
    })
    .finally(() => {
      form.loading2 = false
    })
