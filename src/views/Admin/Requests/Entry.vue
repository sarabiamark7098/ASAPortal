<template>
  <div class="p-4 sm:p-6">
    <FullScreenLoader :visible="form.loading" message="Loading Entry to DSWD Premises Request..." />

    <div v-if="!form.loading">
      <!-- Header -->
      <div class="mb-4">
        <h1 class="text-2xl sm:text-3xl font-bold">Entry to DSWD Premises Requests</h1>
...
import PrintTransaction from '@/views/Admin/Requests/EntryRequest/PrintTransaction.vue'
...
const EntryRequests = computed(() => form.entryRequests)
...
function loadRequests(page = currentPage.value) {
  ...
  form
    .getEntryRequests(authStore.token, page, form.rows, search, sortBy, sortDir)
    .catch((err) => {
      console.error('Failed to load Entry to DSWD Premises requests:', err)
    })
    .finally(() => {
      form.loading2 = false
    })
